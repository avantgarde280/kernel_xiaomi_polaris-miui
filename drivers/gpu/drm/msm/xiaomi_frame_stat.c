...
void calc_fps(u64 duration, int input_event)
{
	...
	if (fm_stat.last_frame_commit_time_us.tv64 > 0) {
	...
	if (diff_us >= FPS_PERIOD_1_SEC) {
		...
		pr_debug("FPS is %d.%d  max_frame_duration = %lld(us) max_input_fence_duration = %lld(us)\n",
				(unsigned int)fps/10,
				(unsigned int)fps%10,
				fm_stat.max_frame_duration.tv64/NANO_TO_MICRO,
				fm_stat.max_input_fence_duration.tv64/NANO_TO_MICRO);
		goto exit;
	} else {
		fm_stat.delta_commit_duration.tv64 = duration;
		if (fm_stat.max_frame_duration.tv64 < fm_stat.delta_commit_duration.tv64)
			fm_stat.max_frame_duration.tv64 = fm_stat.delta_commit_duration.tv64;

		fm_stat.delta_input_duration.tv64 = fm_stat.input_fence_duration;
		if (fm_stat.max_input_fence_duration.tv64 < fm_stat.delta_input_duration.tv64)
			fm_stat.max_input_fence_duration.tv64 = fm_stat.delta_input_duration.tv64;
...
exit:
	...
	fm_stat.start = false;
	fm_stat.max_frame_duration.tv64 = 0;
	fm_stat.max_input_fence_duration.tv64 = 0;
	fm_stat.last_frame_commit_time_us = current_time_us;
final:
	return;
}
